...
%>
    ...
    --></script>
      ...
      <table width="100%" border="0" name="<%=id %>" style="<bean:write name="thisSelect" property="styleDisplay" />">
      <tr>
        <td width="35%"/>
        <td width="20%"/>
      <td width="35%" align="left" rowspan="<%=2 * selectListKeys.length + 1%>">
        <input name="<%=id %>.group.all" class="btn_group_n1" value="<gsmsg:write key="cmn.sel.all.group" />" onclick="" type="button"><br>
          <html:select  property="<%=id + \".group.selectedSingle\" %>" style=" max-width:150px;">
          ...
          <input  type="button"  class="group_btn2" value="&nbsp;&nbsp;" name="<%=id %>.group.select" /><br />
        <html:select property="<%=id + \".targetNoSelected\" %>" size="13" styleClass="select01 noSelected" multiple="true">
      ...
      <tr name="<%=id%>.selected(<%=selectListKeys[0]%>).head" >
      </logic:equal>
      <logic:notEqual name="thisSelect" property="<%=\"selectedListUse.\" + selectListKeys[0]%>" value="true">
      <tr name="<%=id%>.selected(<%=selectListKeys[0]%>).head" style="display: none">
      </logic:notEqual>
      ...
      <logic:notEqual name="thisSelect" property="<%=\"selectedListUse.\" + selectListKeys[0]%>" value="true">
      <tr name="<%=id%>.selected(<%=selectListKeys[0]%>)" style="display: none">
      </logic:notEqual>
      <td align="center">
        ...
        <html:select property="<%=id + \".targetSelected\" %>" size="<%= String.valueOf(selectSize) %>" styleClass="select01 selected" multiple="true" >
      ...
      <td align="center">
        <input type="button" class="btn_base1add" value="<gsmsg:write key="cmn.add"/>" name="adduserBtn" ><br><br>
        <input type="button" class="btn_base1del" value="<gsmsg:write key="cmn.delete" />" name="deluserBtn" >
      </td>
      </tr>
      <% if (selectListKeys.length > 1) {%>

      <logic:equal name="thisSelect" property="<%=\"selectedListUse.\" + selectListKeys[1]%>" value="true">
      <tr name="<%=id%>.selected(<%=selectListKeys[1]%>).head" >
      </logic:equal>
      <logic:notEqual name="thisSelect" property="<%=\"selectedListUse.\" + selectListKeys[1]%>" value="true">
      <tr name="<%=id%>.selected(<%=selectListKeys[1]%>).head" style="display: none">
      </logic:notEqual>
      ...
      <logic:notEqual name="thisSelect" property="<%=\"selectedListUse.\" + selectListKeys[1]%>" value="true">
      <tr name="<%=id%>.selected(<%=selectListKeys[1]%>)" style="display: none">
      </logic:notEqual>
